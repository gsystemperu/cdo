ix = 0;
ult_id = 0;
Ext.define("MyDesktop.app.views.ContratoModificarNota", {
  extend : 'Ext.window.Window',
  alias : 'widget.contratomodificarnota',
  width : 650,
  height : 830,
  modal : true,
    ...
    Ext.Ajax.request({
      ...
      success : function(conn, response, options, eOpts) {
        ...
        Ext.each(result.items, function(item) {
          ...
          } else if (item.tipdoc == 'NP' || item.tipdoc == 'N') {
        ...
        Ext.Ajax.request({
          ...
          success : function(conn, response, options, eOpts) {
            ...
            store.sync();
            grid.getEl().unmask();
            Ext.getCmp('contratomodificarnota').getCalcularContrato();
          }
        });
      }
    });
      ...
      success : function(conn, response, options, eOpts) {
        ...
        if (result.success) {
          Ext.each(result.items, function(item) {
            ...
            ActualizarContratoDetalleModificarNota(item.ERROR, store);
          });
        } else {
...
function ActualizarContratoDetalleModificarNota(_idcontrato, _store) {
  ...
  fx_ContratoGuardarDetalleModificarNota(_idcontrato, idproductos, cantidades,
                                     medidas1, medidas2, totales, idmateriales,
                                     bobina, operario, obseritem, textoitem);
}
function fx_ContratoGuardarDetalleModificarNota(
