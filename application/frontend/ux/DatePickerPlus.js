...
//calculate the HTML of one month at first to gain some speed when rendering many calendars
        ...
        this.keyNav = new Ext.KeyNav(this.eventEl, {
            ...
            "enter" : function(e){
            	// Nilesh - Fixes event propogation issue for ENTER key.
            	// This is bug with Extjs 2.1 and datepicker. 
                //e.stopPropagation(); // - Do NOT stop event propogation - Pnet needs this event to send details to server side!
				if (!this.disabled) {				
		...
		for(var x=0,xk=this.noOfMonth;x<xk;++x) {
			...
			if (this.showWeekNumber) {
				...
				while(i < weekNumbers.length) {
					var pnetWorkWeekDate = new Date(wwDate);
					var weekNumber= ((Date.UTC(curWeekStart.getFullYear(), curWeekStart.getMonth(), curWeekStart.getDate(), 0, 0, 0) - Date.UTC(pnetWorkWeekDate.getFullYear(), pnetWorkWeekDate.getMonth(), pnetWorkWeekDate.getDate(), 0, 0, 0))/86400000) ;
					weekNumber = Math.floor(weekNumber/7);
					if (weekNumber>-1 && weekNumber <53){
						weekNumber = weekNumber+1;
				       weekNumbersTextEls[i].innerHTML = weekNumber;
					} else {
						weekNumbersTextEls[i].innerHTML ='-'; 
					}
